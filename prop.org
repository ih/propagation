...
*** projects
**** parsing
write a propagation network for a generative grammar and see if parsing comes for free
***** balanced parentheses language 
S-> SS
S-> 1S0
S-> 10
***** forward generation network
****** TODO write as scheme program
...
*** practice
**** parser
try to make a parser using a propagator network
***** design
a grammar is a propagative process
the rules act as propagators connected to a cell that contains the output of derivation
****** an example
...
******* derivation as forward propagation
******** what should the cells be?
perhaps view the string as a list and each element has its own cell
maybe a cell for the output string "a + a - a"
compound cell?
********* TODO read section on side-effects in alexey's thesis
